...
#define VOCAB_TXT_PATH "./model/vocab.txt"

int init_cn_clip_model(const char* img_model_path, const char* text_model_path, rknn_app_context_t* app_ctx)
{
    int ret;

    printf("--> init cn_clip image model\n");
    ret = init_clip_model_utils(&(app_ctx->img), img_model_path);
    if (ret < 0)
    {
        printf("rknn_init cn_clip image model fail! ret=%d\n", ret);
        return -1;
    }

    printf("--> init cn_clip text model\n");
    ret = init_clip_model_utils(&(app_ctx->text), text_model_path);
    if (ret < 0)
    {
        printf("rknn_init cn_clip text model fail! ret=%d\n", ret);
        return -1;
    }
...
int release_cn_clip_model(rknn_app_context_t* app_ctx)
{
    ...
    delete app_ctx->cn_clip_tokenize;

    return 0;
}

int inference_cn_clip_model(rknn_app_context_t* app_ctx, image_buffer_t* img, char** input_texts, int text_num, clip_res* out_res)
{
    int ret;
    int* tokens;
    ...
    app_ctx->cn_clip_tokenize = new ChineseCLIPTokenizer(VOCAB_TXT_PATH, sequence_len);
    ...
    {
        std::vector<int> input_ids = app_ctx->cn_clip_tokenize->tokenize(input_texts[i]);
        for (int j = 0; j < input_ids.size(); j++)
        {
            tokens[i*sequence_len+j] = input_ids[j];
        }
    }

    printf("--> inference cn_clip image model\n");

    ret = inference_clip_image_model_utils(&(app_ctx->img), img, img_output);
    if (ret != 0)
    {
        printf("inference cn_clip image model fail! ret=%d\n", ret);
        goto out;
    }

    printf("--> inference cn_clip text model\n");
    for (int i = 0; i < text_num; i++)
    {   
        ret = inference_cn_clip_text_model_utils(&(app_ctx->text), tokens + (i*sequence_len), text_output + (i*app_ctx->text.output_attrs[0].dims[1]));
        if (ret != 0)
        {
            printf("inference cn_clip text model fail! ret=%d\n", ret);
            goto out;
        }
    }
