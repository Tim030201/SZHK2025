// refer to https://github.com/ericperfect/libtorch_tokenizer
#include <fstream>
#include <iostream>
#include <string>
#include <vector>
...
void WordpieceTokenizer::init(std::map<std::string, int> vocab)
{
    vocab_ = vocab;
    unk_token_ = "[UNK]";
...
vector<int> ChineseCLIPTokenizer::tokenize(std::string text)
{
    vector<std::string> split_tokens;
    ...
    if (split_tokens.size() > max_seq_length - 2)
    {
        split_tokens.assign(split_tokens.begin(), split_tokens.begin() + max_seq_length - 2);
    }

    split_tokens.insert(split_tokens.begin(), "[CLS]");
    split_tokens.push_back("[SEP]");

    std::vector<int>  ids = this->convert_tokens_to_ids(split_tokens);

    while (ids.size() < max_seq_length)
    {
        ids.push_back(0);
    }
...
vector<int> ChineseCLIPTokenizer::convert_tokens_to_ids(vector<std::string> tokens)
{
    vector<int> ids;
    vector<std::string>::iterator ptr;
